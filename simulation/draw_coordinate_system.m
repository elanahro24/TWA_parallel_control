...
[X,Y,Z]=draw_cone(cone_height,cone_base_radius,numpoints,Rx,x_cone_origin);
h=surf(X,Y,Z);
set(h,'facecolor',colorstring(1),'FaceAlpha',0.4);
...
[X,Y,Z]=draw_cone(cone_height,cone_base_radius,numpoints,Ry,y_cone_origin);
h=surf(X,Y,Z);
set(h,'facecolor',colorstring(2),'FaceAlpha',0.4);

%drawing the cone for z_axis%
Rz=R;
z_cone_origin=[origin(1)+z_axis(1); origin(2)+z_axis(2); origin(3)+z_axis(3)];
[X,Y,Z]=draw_cone(cone_height,cone_base_radius,numpoints,R,z_cone_origin);
h=surf(X,Y,Z);
set(h,'facecolor',colorstring(3),'FaceAlpha',0.4);
...
function [X,Y,Z]=draw_cone(cone_height,cone_base_radius,numpoints,R,cone_origin)
