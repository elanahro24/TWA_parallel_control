function [des_macro_len,micro_len_out,p_in_w, b_in_w]  = getleglen(x_des,...
    ee_rot,p_in_m,b_in_w,m_in_w,f_in_w,t_in_w,twa_gamma,use_twa,macro_len_prev,...
    micro_len)
    ...
    for i = 1:3
        ...
        if check >= -1 && check <= 1
            ...
            for j = 1:2
                ...
                if beta(1,j) >= -4*pi/180 && beta(1,j) <= 4*pi/180
                    beta_cmd = beta(1,j);
                end
            end
        end
        ...
        micro_len_out(i,1) = sqrt(a^2 + d^2 - 2*a*d*cos(delta - twa_gamma)) - 16.7;
